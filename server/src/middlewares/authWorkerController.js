 ...
 * @description Verifica el token de autenticación y permite el acceso si es válido y tiene rol de 'administrador' o empleado.
 * @env {string} SECRET - La clave secreta para verificar el token de autenticación.
 */
const authWorkerController = (req, res, next) => {
    try {
        ...
        try {
            ...
            if (
                tokenInfo.role !== 'administrador' &&
                tokenInfo.role !== 'empleado'
            )
                generateErrorUtil(
...
export default authWorkerController;
