 ...
 */
const getLineIdByNumberModel = async (type, serie, number, line) => {
    //ha pasado por Joi, así que sabemos que tiene todos los valores obligatorios como mínimo
    const pool = await getPool();

    const [res] = await pool.query(
        'SELECT id FROM invoice_headers WHERE type = ? AND series = ? AND number = ? AND line = ?',
        [type, serie, number, line],
    );
